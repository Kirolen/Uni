...
void ProcessInitialization(int *&pMatrix, int &Size, bool isTest);
...
int main(int argc, char* argv[]) {
    ...
    if (mode == 0) {
        // Default mode
        ProcessInitialization(pMatrix, Size, false);
        ...
        // Test mode
        int testSizes[] = {10, 500, 600, 700, 800, 900, 1000};
        ...
        for (int i = 0; i < numTests; i++) {
            Size = testSizes[i];
            printf("\n--- Test with Size = %d ---\n", Size);
            ProcessInitialization(pMatrix, Size, true);
            start = clock();
            SerialFloyd(pMatrix, Size);
            finish = clock();
            duration = (finish - start) / double(CLOCKS_PER_SEC);
